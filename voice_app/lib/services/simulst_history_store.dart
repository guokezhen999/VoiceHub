...
/// Persistent store manager for Simultaneous Translation (simulst) history.
class SimulstHistoryStore {
  static final _rand = Random();

  static Future<Directory> _historyDir() async {
    final appSupport = await getApplicationSupportDirectory();
    final dir = Directory(p.join(appSupport.path, 'history', 'simulst'));
    if (!await dir.exists()) {
  ...
  static String newId() {
    ...
    return 'simulst-$stamp-$suffix';
  }

  /// Lists all simultaneous translation history summaries, sorted by updatedAt descending.
  ...
  /// Calculates total size of all files in simulst history store.
  ...
  /// Clears all history sessions and audio files in simulst history store.
