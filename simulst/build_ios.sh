#!/bin/bash
# Build libsimulst for iOS (dynamic library + xcframework).
#
# Prerequisites:
...
#
# Output:
#   simulst/flutter/simulst_macos/ios/simulst.xcframework/
...
FLUTTER_IOS_DIR="${SCRIPT_DIR}/flutter/simulst_macos/ios"
...
build_target() {
  ...
  echo "Done: build-${build_subdir}/libsimulst.dylib"
}
...
if [ ! -f build-sim_x86_64/libsimulst.dylib ]; then
...
if [ ! -f build-sim_arm64/libsimulst.dylib ]; then
...
if [ ! -f build-os64/libsimulst.dylib ]; then
...
cp build-os64/libsimulst.dylib ios-arm64/libsimulst.dylib
cp build-sim_arm64/libsimulst.dylib ios-arm64-simulator/libsimulst.dylib
cp build-sim_x86_64/libsimulst.dylib ios-x86_64-simulator/libsimulst.dylib

# Combine simulator slices using lipo
echo "Creating universal binary for simulator..."
mkdir -p ios-arm64_x86_64-simulator
lipo -create \
  ios-arm64-simulator/libsimulst.dylib \
  ios-x86_64-simulator/libsimulst.dylib \
  -output ios-arm64_x86_64-simulator/libsimulst.dylib
...
create_framework() {
  ...
  cp "libsimulst.dylib" "${name}.framework/${name}"
...
<dict>
  ...
  <string>com.voicehub.simulst</string>
...
create_framework "ios-arm64" "simulst"
create_framework "ios-arm64_x86_64-simulator" "simulst"

# Create xcframework
rm -rf simulst.xcframework
xcodebuild -create-xcframework \
  -framework ios-arm64/simulst.framework \
  -framework ios-arm64_x86_64-simulator/simulst.framework \
  -output simulst.xcframework

echo "xcframework created at: ${BUILD_DIR}/simulst.xcframework"

# ---- Copy to Flutter plugin --------------------------------------------------

mkdir -p "${FLUTTER_IOS_DIR}"
rm -rf "${FLUTTER_IOS_DIR}/simulst.xcframework"
cp -R simulst.xcframework "${FLUTTER_IOS_DIR}/"

echo ""
echo "========================================"
echo "Build succeeded!"
echo "xcframework: ${FLUTTER_IOS_DIR}/simulst.xcframework"
echo "========================================"
