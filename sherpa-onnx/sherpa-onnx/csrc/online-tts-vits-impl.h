// sherpa-onnx/csrc/online-tts-vits-impl.h
//
// Copyright (c)  2023  Xiaomi Corporation
#ifndef SHERPA_ONNX_CSRC_ONLINE_TTS_VITS_IMPL_H_
#define SHERPA_ONNX_CSRC_ONLINE_TTS_VITS_IMPL_H_
...
class OnlineTtsVitsImpl : public OfflineTtsImpl {
 public:
  explicit OnlineTtsVitsImpl(const OfflineTtsConfig &config)
      : config_(config),
        model_(std::make_unique<OnlineTtsVitsModel>(config.model)) {
    InitFrontend();
  ...
  OnlineTtsVitsImpl(Manager *mgr, const OfflineTtsConfig &config)
      : config_(config),
        model_(std::make_unique<OnlineTtsVitsModel>(mgr, config.model)) {
    InitFrontend(mgr);
  ...
  GeneratedAudio Generate(
    ...
    try {
      token_ids = frontend_->ConvertTextToTokenIds(text, meta_data.voice);
    } catch (const std::exception &e) {
      SHERPA_ONNX_LOGE("ConvertTextToTokenIds CRASHED: %s", e.what());
      throw;
    }
    ...
    bool has_tones = false;
    for (const auto &i : token_ids) {
      if (!i.tokens.empty() && !i.tones.empty()) {
        has_tones = true;
        break;
      }
    }
    ...
    if (x.empty()) {
      SHERPA_ONNX_LOGE("Failed to convert %s to valid token IDs", text.c_str());
      return {};
    }

    // TODO(fangjun): add blank inside the frontend, not here
    if (meta_data.add_blank && config_.model.online_vits.data_dir.empty() &&
    ...
    if (config_.max_num_sentences <= 0 || x_size <= config_.max_num_sentences) {
      auto ans = Process(x, tones, sid, speed, gen_config.silence_scale, callback);
      if (callback && !(config_.model.online_vits.stream && tones.empty())) {
    ...
    for (int32_t b = 0; b != num_batches && should_continue; ++b) {
      ...
      auto audio =
          Process(batch_x, batch_tones, sid, speed, gen_config.silence_scale, callback);
 ...
 private:
  ...
  void InitFrontend(Manager *mgr) {
    ...
    SHERPA_ONNX_LOGE(
        "VITS Metadata (mgr): frontend=%s, jieba=%d, use_g2pw=%d, language=%s, "
        "voice=%s, is_piper=%d, is_melo_tts=%d, punctuations=%s",
        meta_data.frontend.c_str(), meta_data.jieba, meta_data.use_g2pw,
        meta_data.language.c_str(), meta_data.voice.c_str(),
        meta_data.is_piper, meta_data.is_melo_tts,
        meta_data.punctuations.c_str());

    if (meta_data.frontend == "characters") {
      frontend_ = std::make_unique<OfflineTtsCharacterFrontend>(
          mgr, config_.model.online_vits.tokens, meta_data);
    } else if (meta_data.jieba && meta_data.is_melo_tts) {
      frontend_ = std::make_unique<MeloTtsLexicon>(
          mgr, config_.model.online_vits.lexicon, config_.model.online_vits.tokens,
      ...
      frontend_ = std::make_unique<MeloTtsLexicon>(
          ...
          model_->GetMetaData(), config_.model.debug);
               ...
               !config_.model.online_vits.data_dir.empty()) {
      frontend_ = std::make_unique<OnlinePiperPhonemizeLexicon>(
          mgr, config_.model.online_vits.tokens, config_.model.online_vits.data_dir,
          meta_data);
    } else {
      if (config_.model.online_vits.lexicon.empty()) {
        SHERPA_ONNX_LOGE(
  ...
  void InitFrontend() {
    ...
    SHERPA_ONNX_LOGE(
        "VITS Metadata: frontend=%s, jieba=%d, use_g2pw=%d, language=%s, "
        "voice=%s, is_piper=%d, is_melo_tts=%d, punctuations=%s",
        meta_data.frontend.c_str(), meta_data.jieba, meta_data.use_g2pw,
        meta_data.language.c_str(), meta_data.voice.c_str(),
        meta_data.is_piper, meta_data.is_melo_tts,
    ...
    if (meta_data.frontend == "characters") {
      frontend_ = std::make_unique<OfflineTtsCharacterFrontend>(
          config_.model.online_vits.tokens, meta_data);
    } else if (meta_data.jieba && meta_data.is_melo_tts) {
      frontend_ = std::make_unique<MeloTtsLexicon>(
          config_.model.online_vits.lexicon, config_.model.online_vits.tokens,
          model_->GetMetaData(), config_.model.debug);
    } else if (meta_data.is_melo_tts && meta_data.language == "English") {
      frontend_ = std::make_unique<MeloTtsLexicon>(
          config_.model.online_vits.lexicon, config_.model.online_vits.tokens,
      ...
      frontend_ = std::make_unique<CharacterLexicon>(
          ...
          config_.model.debug, meta_data.use_g2pw);
               ...
               !config_.model.online_vits.data_dir.empty()) {
      frontend_ = std::make_unique<OnlinePiperPhonemizeLexicon>(
          config_.model.online_vits.tokens, config_.model.online_vits.data_dir,
          model_->GetMetaData());
    } else {
      if (config_.model.online_vits.lexicon.empty()) {
        SHERPA_ONNX_LOGE(
  ...
  GeneratedAudio Process(const std::vector<std::vector<int64_t>> &tokens,
    ...
    GeneratedAudio ans;
    if (tones.empty()) {
      OnlineTtsVitsModel::GeneratedAudioCallbackInnel wrapper_cb = nullptr;
      if (callback) {
        wrapper_cb = [&callback](const GeneratedAudio &audio_chunk, float progress) {
          return callback(audio_chunk.samples.data(), audio_chunk.samples.size(), progress) != 0;
        };
      }
      ans = model_->Run(std::move(x_tensor), sid, speed, wrapper_cb);
    } else {
      Ort::Value audio = model_->Run(std::move(x_tensor), std::move(tones_tensor), sid, speed);
 ...
 private:
  ...
  std::unique_ptr<OnlineTtsVitsModel> model_;
  std::vector<std::unique_ptr<kaldifst::TextNormalizer>> tn_list_;
  std::unique_ptr<OfflineTtsFrontend> frontend_;
};

}  // namespace sherpa_onnx
#endif  // SHERPA_ONNX_CSRC_ONLINE_TTS_VITS_IMPL_H_
