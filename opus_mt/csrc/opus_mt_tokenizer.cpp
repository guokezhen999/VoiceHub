...
bool OpusMtTokenizer::LoadVocab(const std::string& vocab_json_path) {
...
bool OpusMtTokenizer::LoadSourceSpm(const std::string& spm_path) {
  source_spm_path_ = spm_path;
#ifdef OPUS_MT_USE_SENTENCEPIECE
  source_spm_ = std::make_unique<sentencepiece::SentencePieceProcessor>();
  auto status = source_spm_->Load(spm_path);
  if (!status.ok()) {
    fprintf(stderr, "[opus-mt] Failed to load source.spm: %s\n",
            status.ToString().c_str());
    source_spm_.reset();
    return false;
  }
  fprintf(stderr, "[opus-mt] Loaded source.spm OK\n");
#endif
  return true;
}

bool OpusMtTokenizer::LoadTargetSpm(const std::string& spm_path) {
  target_spm_path_ = spm_path;
#ifdef OPUS_MT_USE_SENTENCEPIECE
  target_spm_ = std::make_unique<sentencepiece::SentencePieceProcessor>();
  auto status = target_spm_->Load(spm_path);
  if (!status.ok()) {
    fprintf(stderr, "[opus-mt] Failed to load target.spm: %s\n",
            status.ToString().c_str());
    target_spm_.reset();
    return false;
  }
  fprintf(stderr, "[opus-mt] Loaded target.spm OK\n");
#endif
  return true;
}

void OpusMtTokenizer::BuildTrie() {
  delete trie_root_;
...
std::vector<std::string> OpusMtTokenizer::PreTokenize(std::string_view text) const {
...
int32_t OpusMtTokenizer::FindLongestMatch(const std::string& text, size_t pos) const {
  TrieNode* node = trie_root_;
  int32_t best_id = -1;
...
std::vector<int32_t> OpusMtTokenizer::EncodeWithVocab(std::string_view text,
                                                       int32_t bos_id,
...
std::vector<int32_t> OpusMtTokenizer::EncodeWithSentencePiece(std::string_view text,
                                                               int32_t bos_id,
                                                               int32_t eos_id) const {
#ifdef OPUS_MT_USE_SENTENCEPIECE
  if (source_spm_) {
    // Use source SPM to tokenize into piece STRINGS (not IDs).
    // The SPM has its own ID space that is different from vocab.json IDs
    // (which are the actual model token IDs). We must map through vocab_.
    std::vector<std::string> pieces;
    auto status = source_spm_->Encode(std::string(text), &pieces);
    if (!status.ok()) {
      fprintf(stderr, "[opus-mt] SentencePiece encode error: %s\n",
              status.ToString().c_str());
      return {};
    }

    std::vector<int32_t> result;
    // NOTE: Marian models (Helsinki-NLP/opus-mt) do NOT prepend BOS.
    // Only EOS is appended at the end, matching HuggingFace behavior.

    for (const auto& piece : pieces) {
      auto it = vocab_.find(piece);
      if (it != vocab_.end()) {
        result.push_back(it->second);  // vocab.json model ID
      } else {
...
std::vector<int32_t> OpusMtTokenizer::Encode(std::string_view text,
                                              int32_t bos_id,
                                              int32_t eos_id) const {
...
bool OpusMtTokenizer::IsPunctuation(const std::string& token) {
  if (token.size() == 1) {
    char c = token[0];
    return c == '.' || c == ',' || c == '!' || c == '?' || c == ':' ||
           c == ';' || c == '-' || c == '\'' || c == '"' || c == ')' ||
...
std::string OpusMtTokenizer::Decode(const std::vector<int32_t>& ids) const {
#ifdef OPUS_MT_USE_SENTENCEPIECE
  if (target_spm_) {
    // Model output IDs are vocab.json IDs, NOT target SPM IDs.
    // Map through id_to_token_ to get piece strings, then decode with target SPM.
    std::vector<std::string> pieces;
    for (int32_t id : ids) {
      if (id < 0 || id >= static_cast<int32_t>(id_to_token_.size())) continue;
      const std::string& token = id_to_token_[id];

      // Skip special tokens that should not be detokenized.
      if (token == "</s>" || token == "<eos>" || token == "<pad>") continue;
      if (token.size() >= 2 && token.front() == '<' && token.back() == '>') continue;
      // Skip language tokens (">>zh<<" etc.).
      if (token.size() >= 2 && token.substr(0, 2) == ">>") continue;

      pieces.push_back(token);
    }

    std::string result;
    auto status = target_spm_->Decode(pieces, &result);
    if (status.ok()) return result;

    fprintf(stderr, "[opus-mt] SentencePiece decode error: %s\n",
            status.ToString().c_str());
    // Fall through to vocab-based decoding on error.
  }
#endif
