...
Exports:
  - encoder_model.onnx         : standalone encoder
...
class DecoderInitOnnx(nn.Module):
    ...
    def forward(
        ...
        out = self.decoder(
            ...
            past_key_values=None,        # ← first step: compute everything
            use_cache=True,
            return_dict=True,
        )
        ...
        for i in range(self.num_layers):
            results.append(cache.self_attention_cache[i][0])   # decoder key
            results.append(cache.self_attention_cache[i][1])   # decoder value
            results.append(cache.cross_attention_cache[i][0])  # encoder key   ← NEW!
            results.append(cache.cross_attention_cache[i][1])  # encoder value ← NEW!
        return tuple(results)


class DecoderStepOnnx(nn.Module):
    """Incremental-step decoder: uses pre-computed cross-attention KVs
    from the first step. Does NOT take encoder_hidden_states."""
    ...
    def forward(
        ...
        input_ids: torch.Tensor,                  # [batch, 1]
        encoder_attention_mask: torch.Tensor,     # [batch, enc_seq]
        *past_kvs: torch.Tensor,                  # 4 * num_layers tensors (dk, dv, ek, ev per layer)
    ):
        ...
        out = self.decoder(
            input_ids=input_ids,
            encoder_hidden_states=None,       # ← not needed: cross-attn KVs are cached
            encoder_attention_mask=encoder_attention_mask,
            past_key_values=pkv,
            use_cache=True,
            return_dict=True,
        ...
        results = [logits]
        for i in range(self.num_layers):
            results.append(cache.self_attention_cache[i][0])   # decoder key (updated)
            results.append(cache.self_attention_cache[i][1])   # decoder value (updated)
            results.append(cache.cross_attention_cache[i][0])  # encoder key (same)
            results.append(cache.cross_attention_cache[i][1])  # encoder value (same)
        return tuple(results)
...
def export_decoder_step(model, output_path: Path, enc_seq_len: int = 16):
    """Export incremental-step decoder (with past KV, no encoder_hidden_states)."""
    num_layers = model.config.decoder_layers
    ...
    dummy_kv_list = []
    for i in range(num_layers):
        dummy_kv_list.append(torch.randn(1, num_heads, dummy_past_len, head_dim))   # dk
        dummy_kv_list.append(torch.randn(1, num_heads, dummy_past_len, head_dim))   # dv
        dummy_kv_list.append(torch.randn(1, num_heads, enc_seq_len, head_dim))      # ek
        dummy_kv_list.append(torch.randn(1, num_heads, enc_seq_len, head_dim))      # ev

    input_names = ["input_ids", "encoder_attention_mask"] + _build_kv_names("past_key_values", num_layers)
    output_names = ["logits"] + _build_kv_names("present", num_layers)
    ...
    for i in range(num_layers):
        ...
        for kt in ("encoder.key", "encoder.value"):
            ...
            dynamic_axes[f"present.{i}.{kt}"] = {0: "batch_size", 2: "encoder_sequence_length"}

    print(f"      Tracing decoder_step (enc_seq={enc_seq_len}, past_len={dummy_past_len}) ...")
    torch.onnx.export(
        wrapper,
        (dummy_ids, dummy_mask, *dummy_kv_list),
        str(output_path),
        input_names=input_names,
        output_names=output_names,
        dynamic_axes=dynamic_axes,
...
def main():
    ...
    print(f"[1/6] Loading model from {input_dir} ...")
    ...
    export_encoder(model, output_dir / "encoder_model.onnx", args.enc_seq_len)

    print("[3/6] Exporting decoder_init (first step, computes cross-attn KVs) ...")
    export_decoder_init(model, output_dir / "decoder_model_init.onnx", args.enc_seq_len)

    print("[4/6] Exporting decoder_step (incremental, reuses cross-attn KVs) ...")
    export_decoder_step(model, output_dir / "decoder_model.onnx", args.enc_seq_len)

    print("[5/6] Copying vocab and SentencePiece files ...")
    copy_vocab_and_spm(input_dir, output_dir)

    print("[6/6] Writing config.json ...")
